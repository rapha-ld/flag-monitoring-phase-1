...
const MetricsModal = ({
  ...
  const handleMetricToggle = (metricId: string) => {
    setLocalSelectedMetrics(current => current.includes(metricId) ? current.filter(id => id !== metricId) : [...current, metricId]);
  };
  const handleSave = () => {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader className="flex flex-row items-center justify-between my-0 py-[9px]">
        ...
        <MetricSearch searchQuery={searchQuery} onSearchChange={setSearchQuery} />

        <MetricsList metricCategories={metricCategories} recentlyUsedMetrics={recentlyUsedMetrics} selectedMetrics={localSelectedMetrics} onMetricToggle={handleMetricToggle} searchQuery={searchQuery} />
