...
const DashboardMetrics: React.FC<DashboardMetricsProps> = ({
  ...
  // This state tracks whether the Unique Users chart has its breakdown enabled
  const [hasEvaluationBreakdown, setHasEvaluationBreakdown] = useState(false);

  // Function to be passed to the MetricCard to track its breakdown state
  const handleBreakdownChange = (metricType: string, enabled: boolean) => {
    if (metricType === 'evaluations') {
      setHasEvaluationBreakdown(enabled);
    }
  };

  return (
    <div className={`grid gap-4 ${hasEvaluationBreakdown ? 'grid-cols-3' : 'grid-cols-1 md:grid-cols-3'}`}>
      {selectedMetrics.includes('evaluations') && (
        <div className={`${hasEvaluationBreakdown ? 'col-span-2' : ''}`}>
          <MetricCard 
            title="Unique Users" 
            ...
            onBreakdownChange={(enabled) => handleBreakdownChange('evaluations', enabled)}
          />
        </div>
      )}
      
      <div className={`${hasEvaluationBreakdown ? 'col-span-1 flex flex-col gap-4' : ''}`}>
        {selectedMetrics.includes('conversion') && (
          <MetricCard 
            ...
            selectedTimestamp={selectedTimestamp}
            selectedTimestamps={selectedTimestamps}
            onBreakdownChange={(enabled) => handleBreakdownChange('conversion', enabled)}
          />
        )}
        ...
        {selectedMetrics.includes('errorRate') && (
          <MetricCard 
            ...
            selectedTimestamp={selectedTimestamp}
            selectedTimestamps={selectedTimestamps}
            onBreakdownChange={(enabled) => handleBreakdownChange('errorRate', enabled)}
          />
        )}
      </div>
    </div>
